...
func (mess *messages) handle(rcvMess *telegram.Message) {
	...
	if rcvMess.IsCommand() {
		...
		case "movies":
			...
			films, err := radarr.GetFilmsList(mess.radarrConfig)
			if err != nil {
				log.Err(err).Msg("error when getting movies messages")
				editSimpleMessage(mess.bot, rcvMess.Chat.ID, rcvMess.ID, "An error occurred while getting the movies list.\nPlease contact the administrator.")
				return
			}
			...
			sendMessageWithKeyboard(mess.bot, rcvMess.Chat.ID, messages[0]+printPageNum(1, len(messages)), keyboard)
		case "addmovie":
		...
		case "series":
			...
			series, err := sonarr.GetSeriesList(mess.sonarrConfig)
			if err != nil {
				log.Err(err).Msg("error when getting movies messages")
				editSimpleMessage(mess.bot, rcvMess.Chat.ID, rcvMess.ID, "An error occurred while getting the series list.\nPlease contact the administrator.")
				return
			}
		...
		if action, exist := mess.usersAction[rcvMess.From.ID]; exist {
			...
			case "movieDetails":
				...
				sendImageMessage(mess.bot, rcvMess.Chat.ID, film.CoverImage, printMovieTitle(film))
				sendSimpleMessage(mess.bot, rcvMess.Chat.ID, printMovieTitle(film))
			case "serieDetails":
				...
				sendImageMessage(mess.bot, rcvMess.Chat.ID, serie.CoverImage, printSerieTitle(serie))
				sendSimpleMessage(mess.bot, rcvMess.Chat.ID, printSerieTitle(serie))
			default:
